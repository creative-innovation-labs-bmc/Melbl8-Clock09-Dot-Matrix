(() => {
  ...
  async function fetchBomWeather() {
    ...
    // Preferred production path: same-origin weather file generated by GitHub Pages workflow.
    try {
    ...
    // Official Melbourne Olympic Park observation fallback.
    try {
    ...
    function draw(ts) {
      ...
      for (let row=0; row<cfg.rows; row++) {
        for (let col=0; col<cfg.cols; col++) {
          ...
          // Colon animation: a bright pulse travels from the top block to the bottom block every second.
