(() => {
  ...
  const WEATHER_CACHE_KEY = 'clock09-bom-docklands-v1';
  const FPS = 30;
  ...
  function createClock(options={}) {
    const cfg = Object.assign({
      cols:192, rows:36, footerRows:5,
      digitScaleX:3, digitScaleY:2,
      pairGap:4, separatorGap:8, colonWidth:3,
      idleAlpha:0.065, progressAlpha:0.095, activeAlpha:0.98,
      idleMotion:0.012, activeMotion:0.055, scan:0.02,
      activeRadius:1.30, title:''
    }, options);
        ...
        if (pos.type === 'colon') {
          ...
          for (let dx=0; dx<cfg.colonWidth; dx++) {
            for (let dy=0; dy<h; dy++) {
              map.add(`${pos.x+dx},${y1+dy}`); colons.add(`${pos.x+dx},${y1+dy}`);
              map.add(`${pos.x+dx},${y2+dy}`); colons.add(`${pos.x+dx},${y2+dy}`);
            }
          }
          return;
        }
    ...
    function weatherText() {
      if (!weather) return `DOCKLANDS BOM ${weatherState}`;
      ...
      return `DOCKLANDS BOM ${temp} ${humid} ${dir}${speed}`;
    }
    ...
    function rebuildFooter(parts) {
      ...
      if (maxRightStart > leftMargin + lW + 4) {
        ...
        const compact = weather ? `DOCKLANDS ${Math.round(Number(weather.temp||0))}C ${String(weather.direction||'W').toUpperCase()}${Math.round(Number(weather.speed||0))}` : 'DOCKLANDS BOM';
    ...
    function draw(ts) {
      ...
      const colonPulse = 0.72 + 0.28 * ((Math.sin(ts*0.008)+1)/2);
      ...
      for (let row=0; row<cfg.rows; row++) {
        for (let col=0; col<cfg.cols; col++) {
          ...
          if (colonCells.has(key) && isMain) alpha *= colonPulse;
